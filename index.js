```javascript
import { GoogleGenerativeAI } from "@google/genai"; // Correct import
import {
    SAVE_KEY,
    getDefaultGameState,
    gameStyles,
    actions as gameActions, // Import 'actions' and alias to 'gameActions'
    GEMINI_MODEL_TEXT,
    EVENT_REGISTRATION_WINDOW_WEEKS,
    EVENT_REGISTRATION_FEE_BASE,
    EVENT_PRINTING_COST_NEW_BASE_FEE,
    EVENT_PRINTING_COST_RERELEASE,
    LIVING_EXPENSE_AMOUNT,
    LIVING_EXPENSE_INTERVAL_WEEKS,
    PLAYER_CUSTOM_SNS_POST_COOLDOWN_WEEKS,
    LOW_STAMINA_WRITING_THRESHOLD,
    BURNOUT_STREAK_THRESHOLD,
    BURNOUT_CHANCE,
    SOURCE_STAMINA_HEAL_CHANCE,
    SOURCE_STAMINA_HEAL_AMOUNT,
    SOURCE_SKILL_DROP_CHANCE,
    SOURCE_SKILL_DROP_AMOUNT,
    DEFAULT_EVENT_SALE_PRICE,
    OFFICIAL_WARNING_CHANCE_FOR_PAID_CONTENT,
    ALL_GENRES,
    ALL_MATERIALS,
    ALL_COMMENTS_VARIETY,
    PREDEFINED_PLAYER_ANECDOTES,
    SCENARIO_IDEAS_FOR_ANECDOTES,
    QUIRKY_ACTIVITIES_FOR_ANECDOTES,
    ANOTHER_WORK_TITLES_SAMPLE_FOR_ANECDOTES,
    ANOTHER_CHARACTERS_SAMPLE_FOR_ANECDOTES,
    ALTERNATIVE_ENDINGS_SAMPLE_FOR_ANECDOTES,
    UNCONVENTIONAL_MOTIVATIONS_SAMPLE_FOR_ANECDOTES,
    MICRO_FIC_IDEAS_FOR_ANECDOTES,
    MODERN_JOBS_FOR_ANECDOTES,
    timeAgo,
    escapeHTML,
    eventTypeBonus
} from './gameData.js';
...
function updateStatsDisplay() {
    ...
    if (uiElements['stat-stamina'])
        // FIX: Corrected template literal for stamina
        uiElements['stat-stamina'].textContent = `${Math.round(gameState.stamina)} / ${Math.round(gameState.maxStamina)}`;
...
function updateEventDisplay() {
    const eventBar = uiElements['event-bar'];
    const eventNameDisplay = uiElements['event-name-display'];
    const eventFandomDisplay = uiElements['event-fandom-display'];
    const eventDeadlineDisplay = uiElements['event-deadline-display'];
...
function generateNewGameEvent() { // GameEvent type removed for plain JS
    if (Math.random() < 0.25 && gameState.fandomSets.length > 0) {
        ...
        const eventTypes = ["OnlineContest", "LocalFanMeet", "MajorConvention"]; // GameEvent['type'] removed
        const eventNames = {
    ...
    if (!gameState.currentEvent || gameState.currentEvent.eventWeek < gameState.currentWeek) {
        ...
        if (newEvent) {
            ...
            const eventPost = createHNPost(
                "EventBot", `Event: ${newEvent.name} Announced!`, // Simplified title
...
function renderUpcomingEventsList() {
    const listContainer = uiElements['upcoming-events-list'];
    const submissionArea = uiElements['event-submission-fic-selection-area'];
    ...
    if (gameState.currentEvent && gameState.currentEvent.eventWeek >= gameState.currentWeek) {
        ...
        if (gameState.registeredEventId === event.id) {
            ...
            if (!event.submittedFicId) {
                ...
                populateEventFicSubmissionSelect(event.id);
                 ...
                 if(submittedFic && uiElements['event-submission-context-info']) (uiElements['event-submission-context-info']).textContent = `Already submitted: "${escapeHTML(submittedFic.title)}"`;
            }
        ...
        const regButton = entry.querySelector('.register-event-btn');
        if (regButton) {
...
function populateEventFicSubmissionSelect(eventId) {
    const select = uiElements['event-submit-fic-select'];
    const contextInfo = uiElements['event-submission-context-info'];
    ...
    if (eligibleFics.length === 0) {
        ...
        (uiElements['confirm-event-fic-submission-button']).disabled = true;
    ...
    (uiElements['confirm-event-fic-submission-button']).disabled = false;
...
function handleRegisterForEvent(eventId) {
...
function openDoujinshiPrintingModal(ficId, eventId, isReRelease) {
    ...
    (uiElements['doujinshi-target-fic-id']).value = ficId;
    (uiElements['doujinshi-target-event-id']).value = eventId;
    (uiElements['doujinshi-is-rerelease-flag']).value = String(isReRelease);
    (uiElements['doujinshi-fic-title-span']).textContent = escapeHTML(fic.title);

    const pageCount = fic.pageCount || Math.ceil((fic.content?.length || 2000) / 1800);
    (uiElements['doujinshi-estimated-page-count']).textContent = String(pageCount);

    (uiElements['doujinshi-paper-quality']).value = 'Standard';
    (uiElements['doujinshi-cover-quality']).value = 'Color';
    (uiElements['doujinshi-print-run-size']).value = isReRelease ? '20' : '50';
...
function updateDoujinshiCostEstimate() {
    const paperQuality = (uiElements['doujinshi-paper-quality']).value;
    const coverQuality = (uiElements['doujinshi-cover-quality']).value;
    const printRunSize = parseInt((uiElements['doujinshi-print-run-size']).value, 10) || 0;
    const pageCount = parseInt((uiElements['doujinshi-estimated-page-count']).textContent || '0', 10);
    const isReRelease = (uiElements['doujinshi-is-rerelease-flag']).value === 'true';
    const event = gameState.currentEvent;
    ...
    const earlyBirdText = uiElements['doujinshi-early-bird-discount-text'];
    ...
    (uiElements['doujinshi-estimated-printing-cost']).textContent = String(Math.round(totalPrintingCost));
}

function handleConfirmDoujinshiPrinting() {
    const ficId = (uiElements['doujinshi-target-fic-id']).value;
    const eventId = (uiElements['doujinshi-target-event-id']).value;
    const isReRelease = (uiElements['doujinshi-is-rerelease-flag']).value === 'true';
    const paperQuality = (uiElements['doujinshi-paper-quality']).value;
    const coverQuality = (uiElements['doujinshi-cover-quality']).value;
    const printRunSize = parseInt((uiElements['doujinshi-print-run-size']).value, 10);
    let printingCost = parseFloat((uiElements['doujinshi-estimated-printing-cost']).textContent || '0');
    ...
    if (!fic.pageCount) fic.pageCount = parseInt((uiElements['doujinshi-estimated-page-count']).textContent || '10', 10);
...
function handleSubmitToEvent() {
    const selectedFicId = (uiElements['event-submit-fic-select']).value;
...
function setupScheduler() {
    const scheduleDaysContainer = uiElements['schedule-days'];
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        const actionSelectElement = dayDiv.querySelector(`#day-${i}-action`);
        const projectSelectContainer = dayDiv.querySelector('.project-select-container');
        const projectSelectElement = dayDiv.querySelector(`#day-${i}-project`);
        ...
        actionSelectElement.addEventListener('change', (event) => {
            ...
            const currentDayDiv = (event.target).closest('.day-schedule');
            const projSelectContainer = currentDayDiv?.querySelector('.project-select-container');
            const projSelect = currentDayDiv?.querySelector(`.daily-project-select[data-day="${day}"]`);
...
function renderProjectSelectForDay(selectElement, day) {
...
function validateScheduler() {
    const startWeekButton = uiElements['start-week-button'];
    const schedulerValidationMsg = uiElements['scheduler-validation-msg'];
...
function showArea(areaId) {
    ...
    if (areaId) {
        const elementToShow = uiElements[areaId];
        if (elementToShow) {
...
function showNotification(message, type = 'info', duration = 4500) { // Type params removed
    const popup = uiElements['notification-popup'];
    const textElement = uiElements['notification-text'];
...
async function processDayAction(day) { // Promise<void> removed
    return new Promise(resolve => {
        ...
        const dailyPopup = uiElements['daily-popup'];
        const dailyPopupImage = uiElements['daily-popup-image'];
        const dailyPopupText = uiElements['daily-popup-text'];
        const dailyProjectProgressText = uiElements['daily-project-progress-text'];
...
async function handleStartWeek() {
    ...
    if (gameState.currentEvent && gameState.currentWeek > gameState.currentEvent.eventWeek) {
        ...
        const resultPost = createHNPost("EventBot", `Event: ${gameState.currentEvent.name} Concluded!`, eventOutcomeMessage, 20 + Math.floor(Math.random()*20), false);
...
function populateGenreCheckboxes() {
    const container = uiElements['plan-fic-genre-checkboxes'];
    if (!container) return;
    container.innerHTML = '';
    gameState.unlockedGenres.forEach(genre => {
        ...
        checkbox.addEventListener('change', () => {
            const checkedGenres = Array.from(container.querySelectorAll('input[name="fic-genre"]:checked'));
...
function populateMaterialsSelect() {
    const materialsSelect = uiElements['plan-fic-materials'];
    if (materialsSelect) {
...
function handleMaterialSelectionChange() {
    ...
    const sequelGroup = uiElements['plan-fic-sequel-group'];
...
function populatePreviousFicsForSequelDropdown() {
    const dropdown = uiElements['plan-fic-previous-fic-for-sequel'];
    const selectedFandomSetId = (uiElements['plan-fic-fandom-set'])?.value;
...
function populateFandomSetSelect(selectElement) {
...
function openFanficPlanningModal() {
    ...
    populateMaterialsSelect();
    populateFandomSetSelect(uiElements['plan-fic-fandom-set']);
    (uiElements['plan-fic-fandom-set']).removeEventListener('change', populatePreviousFicsForSequelDropdown);
    (uiElements['plan-fic-fandom-set']).addEventListener('change', populatePreviousFicsForSequelDropdown);

    (uiElements['plan-fic-title']).value = '';
    (uiElements['plan-fic-scenario']).value = '';
    (uiElements['plan-fic-target-progress']).value = '100';
    if (uiElements['plan-fic-sequel-group']) (uiElements['plan-fic-sequel-group']).style.display = 'none';
    showArea('fanfic-planning-modal');
}

function handleStartNewProject() {
    const fandomSetId = (uiElements['plan-fic-fandom-set']).value;
    const title = (uiElements['plan-fic-title']).value.trim();
    const selectedGenreCheckboxes = Array.from(document.querySelectorAll('#plan-fic-genre-checkboxes input[name="fic-genre"]:checked'));
    const genres = selectedGenreCheckboxes.map(cb => cb.value);
    const scenarioPlan = (uiElements['plan-fic-scenario']).value.trim();
    let targetProgress = parseInt((uiElements['plan-fic-target-progress']).value, 10);
    const selectedMaterialsElements = Array.from((uiElements['plan-fic-materials']).selectedOptions);
    const materials = selectedMaterialsElements.map(opt => opt.value).filter(val => val !== "");

    let previousFicIdForSequel = undefined; // removed type
    const sequelDropdown = uiElements['plan-fic-previous-fic-for-sequel'];
    ...
    const newProject = { // ActiveFanficProject type removed
...
function openProjectDetailModal(projectId) {
    ...
    (uiElements['modal-project-title']).textContent = escapeHTML(project.title);
    (uiElements['modal-project-fandom-set-name']).textContent = escapeHTML(projectFandomSet?.workTitle || "Unknown Fandom");
    (uiElements['modal-project-genre']).textContent = escapeHTML(project.genres.join(', '));
    (uiElements['modal-project-materials']).textContent = escapeHTML(project.materials.join(', '));
    (uiElements['modal-project-scenario']).textContent = escapeHTML(project.scenarioPlan);

    const isPublished = 'content' in project; // Check if it's a PublishedFic
    const currentProgress = isPublished ? project.targetProgress : (project).progress; // Removed ActiveFanficProject cast
    (uiElements['modal-project-progress']).textContent = String(currentProgress);
    (uiElements['modal-project-target']).textContent = String(project.targetProgress);

    const sequelInfoDiv = uiElements['modal-project-sequel-info'];
    const prevFicTitleSpan = uiElements['modal-project-previous-fic-title'];
    ...
    const finalizeButton = uiElements['finalize-fic-button'];
    const postButton = uiElements['post-fic-button'];
    const submitToEventButton = uiElements['submit-to-event-button'];
    const outputArea = uiElements['generated-fic-output'];
    const paidCheckbox = uiElements['fic-paid-checkbox'];
    const priceGroup = uiElements['fic-price-group'];
    const priceGroupSmall = uiElements['fic-price-group-small'];
    const priceInput = uiElements['fic-price'];
    ...
    (uiElements['clear-fic-button']).dataset.projectId = project.id;
    ...
    if (activeProject && activeProject.progress >= activeProject.targetProgress && !activeProject.generatedContent) {
        ...
        outputArea.value = activeProject?.generatedContent || (isPublished ? (project).content : "Manuscript in progress..."); // Removed PublishedFic cast
    }
    ...
    if (isPublished) { // If it's a PublishedFic
        ...
        paidCheckbox.checked = (project).isPaid; // Removed PublishedFic cast
        paidCheckbox.disabled = true;
        priceInput.value = String((project).ficPrice || DEFAULT_EVENT_SALE_PRICE); // Removed PublishedFic cast
        priceInput.disabled = true;
        const displayPriceGroup = (project).isPaid ? 'flex' : 'none'; // Removed PublishedFic cast
        if (priceGroup) priceGroup.style.display = displayPriceGroup;
        if (priceGroupSmall) priceGroupSmall.style.display = (project).isPaid ? 'block' : 'none'; // Removed PublishedFic cast
    ...
    (uiElements['fic-loading-indicator']).style.display = 'none';
    showArea('project-detail-modal');
}

async function callGeminiAPI_Actual(
    apiKey, // removed type
    ficPlan // removed type
) { // Promise return type removed
    if (!apiKey) return { success: false, error: "[Config Error] API Key is missing." };
    const ai = new GoogleGenerativeAI({ apiKey }); // FIX: Correct constructor name
...
`;
    ...
    try {
        // Note: Assuming the genai library handles the `generateContent` structure correctly
        // The previous structure `ai.models.generateContent` might be for an older version or specific model type.
        // The common pattern for the Gemini SDK is `ai.getGenerativeModel(...).generateContent(...)`
        const model = ai.getGenerativeModel({ model: GEMINI_MODEL_TEXT, systemInstruction: { parts: [{text: systemInstructionText}]} });
        const result = await model.generateContent(userPromptCore);
        const genAIResponse = await result.response; // Access the response part

        const generatedText = genAIResponse.text(); // Use .text() method
        if (typeof generatedText === 'string') {
            ...
            } else if (genAIResponse.promptFeedback && genAIResponse.promptFeedback.blockReason) {
                 errorMessage = `[API Safety Block] Prompt was blocked. Reason: ${genAIResponse.promptFeedback.blockReason}. Details: ${genAIResponse.promptFeedback.safetyRatings?.map(r => `${r.category} (${r.probability})`).join(', ') || 'N/A'}`;
            }
...
async function handleFinalizeFic(projectId) {
    const project = gameState.fanficProjects.find(p => p.id === projectId);
    ...
    let previousFicFullText = undefined; // Type removed
    let partNumber = 1;
    ...
    (uiElements['fic-loading-indicator']).style.display = 'inline';
    (uiElements['finalize-fic-button']).disabled = true;
    (uiElements['post-fic-button']).disabled = true;
    (uiElements['generated-fic-output']).value = 'AI가 최종 원고를 작성 중입니다... Gemini API를 호출합니다...';
    const ficPlan = {
    ...
    if (result.success && result.text) {
        ...
        (uiElements['generated-fic-output']).value = result.text;
        (uiElements['post-fic-button']).disabled = false;
        ...
        (uiElements['generated-fic-output']).value = `팬픽 생성 오류: ${result.error}\n\nAPI 키, 프롬프트 안전 문제, 또는 Gemini API 서비스 상태를 확인하세요. 요청이 너무 길거나 복잡할 수도 있습니다.`;
        (uiElements['finalize-fic-button']).disabled = false; // Re-enable if failed
...
// --- SNS & Posting ---
function createHNPost(author, title, content, initialLikes, isPaid = false, type = 'post') { // HNPost type removed
...
function createHNComment(postId, author, text) { // HNComment type removed
...
// CommentGenerationOptions type removed
function generateSimulatedComments(post, targetCount, ficRatingInput, options) {
    const comments = []; // HNComment[] removed
...
async function callGeminiAPI_GenerateCommentsForPlayerPost(apiKey, playerPostContent, playerPostTitle) { // types removed
...
`.trim();
    ...
    try {
        // Similar adjustment as callGeminiAPI_Actual for the Gemini SDK
        const model = ai.getGenerativeModel({ model: GEMINI_MODEL_TEXT, systemInstruction: { parts: [{text: systemInstruction}] }});
        const result = await model.generateContent({
            contents: [{role: "user", parts: [{text: userPrompt}]}],
            generationConfig: { responseMimeType: "application/json", temperature: 0.85, topK: 40, topP: 0.95 }
        });
        ...
        // FIX: More robust regex for extracting JSON from (optional) markdown code block
        const fenceRegex = /^```(?:json)?\s*([\s\S]*?)\s*```$/;
        const match = jsonStr.match(fenceRegex);
        if (match && match[1]) {
            jsonStr = match[1].trim();
        } // If no match, assume it's raw JSON (or will fail parsing)
...
async function callGeminiAPI_GeneratePlayerAnecdote(apiKey, anecdoteType) { // types removed
    ...
    try { 
        // Similar adjustment for Gemini SDK
        const model = ai.getGenerativeModel({ model: GEMINI_MODEL_TEXT, systemInstruction: { parts: [{ text: systemInstruction }] } });
        const result = await model.generateContent({
...
function renderHNComment(comment, isReply = false) { // HNComment type removed
    ...
    if (comment.replies && comment.replies.length > 0) {
        const repliesContainer = commentDiv.querySelector('.hn-replies');
...
function renderSNSFeed() {
    const postsContainer = uiElements['posts-container'];
    ...
    sortedPosts.forEach(post => {
        ...
        `;
        const commentsSection = postDiv.querySelector(`#comments-for-${post.id}`);
...
async function handleSubmitPlayerCustomSnsPost() {
    const titleInput = uiElements['player-custom-sns-post-title'];
    const contentInput = uiElements['player-custom-sns-post-content'];
    const submitButton = uiElements['submit-player-custom-sns-post-button'];
    ...
    if (uiElements['player-custom-sns-post-char-count']) (uiElements['player-custom-sns-post-char-count']).textContent = '280 characters remaining';
...
function updatePlayerSnsPostCooldownMessage() {
    const cooldownMessageEl = uiElements['player-custom-sns-post-cooldown-message'];
    const openModalButton = uiElements['open-player-custom-sns-post-modal-button'];
    const submitModalButton = uiElements['submit-player-custom-sns-post-button'];
...
function handlePostFic(projectId) {
    const project = gameState.fanficProjects.find(p => p.id === projectId);
    ...
    const isPaid = (uiElements['fic-paid-checkbox']).checked;
    const ficPrice = parseInt((uiElements['fic-price']).value, 10) || undefined;
    ...
    const newPublishedFic = { // PublishedFic type removed
    ...
    (uiElements['fic-paid-checkbox']).checked = false;
    const priceGroup = uiElements['fic-price-group'];
    const priceGroupSmall = uiElements['fic-price-group-small'];
    if (priceGroup) priceGroup.style.display = 'none';
    if (priceGroupSmall) priceGroupSmall.style.display = 'none';
    (uiElements['fic-price']).value = String(DEFAULT_EVENT_SALE_PRICE);
...
function renderMyFicsList() {
    const myFicsListContainer = uiElements['my-fics-list'];
    ...
    sortedFics.forEach(fic => {
        ...
        const memoTextarea = ficDiv.querySelector(`#memo-${fic.id}`);
        if(memoTextarea) { memoTextarea.addEventListener('change', (e) => { const updatedMemo = (e.target).value; const ficToUpdate = gameState.publishedFics.find(f => f.id === fic.id); if (ficToUpdate) { ficToUpdate.memo = updatedMemo; saveGameState(); } }); }

        const viewDetailsButton = ficDiv.querySelector('.view-fic-details-btn');
...
// --- Profile Edit Logic ---
function findFandomSetById(id) { // FandomSet type removed
    return gameState.fandomSets.find(fs => fs.id === id);
}
function openProfileEditModal() {
    (uiElements['edit-player-name']).value = gameState.playerName;
    (uiElements['edit-api-key']).value = gameState.apiKey;
    renderFandomSetsManagement();
    showArea('profile-edit-area');
}
function renderFandomSetsManagement() {
    const container = uiElements['fandom-sets-management-area'];
    if (!container) return;
    container.innerHTML = '';
    gameState.fandomSets.forEach(fs => {
        ...
        div.querySelector('.edit-fandom-btn')?.addEventListener('click', (e) => handleEditFandomSetForm((e.currentTarget).dataset.id));
        div.querySelector('.delete-fandom-btn')?.addEventListener('click', (e) => handleDeleteFandomSet((e.currentTarget).dataset.id));
    });

    const editForm = uiElements['edit-fandom-set-form'];
    if (editForm && editForm.style.display !== 'none' && !(uiElements['edit-fandom-set-id-input']).value) {
        editForm.style.display = 'none';
    }
}
function handleEditFandomSetForm(fandomSetId) {
    const form = uiElements['edit-fandom-set-form'];
    const formTitle = uiElements['edit-fandom-set-form-title'];
    const idInput = uiElements['edit-fandom-set-id-input'];
    const workTitleInput = uiElements['edit-fandom-work-title-input'];
    const workDescInput = uiElements['edit-fandom-work-desc-input'];
    const favCharInput = uiElements['edit-fandom-fav-char-input'];
    const favCharDescInput = uiElements['edit-fandom-fav-char-desc-input'];
    const favPairingInput = uiElements['edit-fandom-fav-pairing-input'];
    const pairingInterpInput = uiElements['edit-fandom-pairing-interp-input'];
    const isPrimaryCheckbox = uiElements['edit-fandom-is-primary-checkbox'];
    const relationshipTypeSelect = uiElements['edit-fandom-relationship-type'];
    const baseWorkGroup = uiElements['select-base-fandom-set-group'];
    const baseWorkSelect = uiElements['select-base-fandom-set'];
    const popularityTierGroup = uiElements['edit-fandom-popularity-tier-group'];
    const popularityTierSelect = uiElements['edit-fandom-popularity-tier'];
    ...
    const resetAndToggleFields = (relationship) => {
    ...
    const populateBaseWorkSelectAndFields = (currentEditingId, selectedBaseId) => {
    ...
    if (fandomSetId) {
        ...
        [workTitleInput, workDescInput, favCharInput, favCharDescInput, favPairingInput, pairingInterpInput].forEach(el => (el).value = "");
...
function handleSaveFandomSet() {
    const id = (uiElements['edit-fandom-set-id-input']).value;
    let workTitle = (uiElements['edit-fandom-work-title-input']).value.trim();
    let workDescription = (uiElements['edit-fandom-work-desc-input']).value.trim();
    let favCharacter = (uiElements['edit-fandom-fav-char-input']).value.trim();
    let favCharacterDescription = (uiElements['edit-fandom-fav-char-desc-input']).value.trim();
    const favPairing = (uiElements['edit-fandom-fav-pairing-input']).value.trim();
    const pairingInterpretation = (uiElements['edit-fandom-pairing-interp-input']).value.trim();
    const isPrimary = (uiElements['edit-fandom-is-primary-checkbox']).checked;
    const relationshipType = (uiElements['edit-fandom-relationship-type']).value; // FandomSet['relationshipType'] removed
    const baseWorkId = (uiElements['select-base-fandom-set']).value;
    const popularityTier = (uiElements['edit-fandom-popularity-tier']).value; // FandomPopularityTier removed
    let baseFandomSetIdToStore = undefined; // Type removed
    ...
    (uiElements['edit-fandom-set-form']).style.display = 'none';
    renderFandomSetsManagement();
    updateAllDisplays();
    saveGameState();
}

function handleDeleteFandomSet(fandomSetId) {
...
function handleSaveGeneralProfileSettings() {
    const newPlayerName = (uiElements['edit-player-name']).value.trim();
    const newApiKey = (uiElements['edit-api-key']).value.trim();
...
function displayAnalytics() {
    const analyticsContent = uiElements['analytics-content'];
    ...
    const averageRating = playerFicsWithRatings.length > 0 ? (playerFicsWithRatings.reduce((sum, fic) => sum + (fic.readerRating), 0) / playerFicsWithRatings.length).toFixed(1) : 'N/A'; // ! removed
    let genresUsed = {};
...
async function handleSnsActivityEvents(logPrefix = "") { // type removed
    ...
    if(uiElements['sns-feed'] && (uiElements['sns-feed']).style.display === 'flex') { renderSNSFeed(); }
    showNotification(eventMessage, snsEventHappened ? 'success' : 'info', snsEventHappened ? 4000 : 3000);
}

function handleInspirationUnlocks(sourceAction = 'Inspiration') { // type removed
...
function checkSupportiveSenpaiMilestones() {
    ...
    if (milestoneReached && friendPostContent) {
        ...
        if((uiElements['sns-feed'])?.style.display === 'flex') renderSNSFeed();
    }
}

function renderMessageLog() {
    const container = uiElements['message-log-container'];
    if (!container) return;
    container.innerHTML = '';
    ...
    const allLogs = [...gameState.dailyLogs, ...gameState.notificationLog.map(nl => ({ // Combine and map notification logs
        ...
        type: nl.type // Cast to any removed
    }))];
...
function renderManageProjectsList() {
    const container = uiElements['fanfic-projects-list-container'];
    if (!container) return;
    container.innerHTML = '';
    ...
    activeProjects.forEach(project => {
        ...
        const viewButton = projectDiv.querySelector('.open-project-detail-btn');
...
function setupEventListeners() {
    (uiElements['start-game-button'])?.addEventListener('click', () => {
        const apiKey = (uiElements['api-key']).value.trim();
        const playerName = (uiElements['player-name']).value.trim();
        const workTitle = (uiElements['fandom-work-title']).value.trim();
        const favChar = (uiElements['fandom-fav-char']).value.trim();
        const favCharDesc = (uiElements['fandom-fav-char-desc']).value.trim();
        const favPairing = (uiElements['fandom-fav-pairing']).value.trim();
        const storyModeEnabledSetup = (uiElements['initial-story-mode-checkbox']).checked;
        const initialPopTier = (uiElements['initial-fandom-popularity-tier']).value; // FandomPopularityTier removed
        ...
        if (!gameState.gameInitialized) {
            ...
            gameState.fandomSets = [{
                ...
                workDescription: (uiElements['fandom-work-desc']).value.trim(),
                favCharacter: favChar, favCharacterDescription: favCharDesc,
                favPairing, pairingInterpretation: (uiElements['fandom-pairing-interp']).value.trim(),
    ...
    (uiElements['start-week-button'])?.addEventListener('click', handleStartWeek);
    (uiElements['manual-save-button'])?.addEventListener('click', () => {saveGameState(); showNotification("Game Saved!", 'success', 2000);});
    (uiElements['start-over-button'])?.addEventListener('click', startOver);
    (uiElements['clear-schedule-button'])?.addEventListener('click', clearCurrentSchedule);

    (uiElements['manage-projects-button'])?.addEventListener('click', () => {
        renderManageProjectsList();
        showArea('manage-projects-modal');
    });
    (uiElements['plan-new-project-from-manage-button'])?.addEventListener('click', openFanficPlanningModal);

    (uiElements['start-new-project-button'])?.addEventListener('click', handleStartNewProject);
    (uiElements['view-fics-button'])?.addEventListener('click', () => { renderMyFicsList(); showArea('view-fics-area'); });
    (uiElements['view-sns-button'])?.addEventListener('click', () => { renderSNSFeed(); showArea('sns-feed'); });
    (uiElements['analytics-button'])?.addEventListener('click', () => {displayAnalytics(); showArea('analytics-area'); });
    (uiElements['edit-profile-button'])?.addEventListener('click', openProfileEditModal);
    (uiElements['view-upcoming-events-button'])?.addEventListener('click', () => { renderUpcomingEventsList(); showArea('upcoming-events-modal'); });
    (uiElements['confirm-event-fic-submission-button'])?.addEventListener('click', handleSubmitToEvent);

    (uiElements['finalize-fic-button'])?.addEventListener('click', (e) => {
        const projectId = (e.target).dataset.projectId;
        if (projectId) handleFinalizeFic(projectId);
    });
    (uiElements['post-fic-button'])?.addEventListener('click', (e) => {
        const projectId = (e.target).dataset.projectId;
        if (projectId) handlePostFic(projectId);
    });
    (uiElements['submit-to-event-button'])?.addEventListener('click', (e) => {
        const projectId = (e.target).dataset.projectId;
        const project = gameState.fanficProjects.find(p => p.id === projectId) || gameState.publishedFics.find(f => f.id === projectId);
        if (project && ( (project).generatedContent || (project).content) && gameState.currentEvent) { // removed ActiveFanficProject/PublishedFic casts
            ...
            if (ficToSubmit) {
                ...
                // If it's an active project that's finalized, it means it's not yet "published" in gameState.publishedFics
                // So it's a new print for the event.
    ...
    (uiElements['clear-fic-button'])?.addEventListener('click', (e) => {
        const projectId = (e.target).dataset.projectId;
        const project = gameState.fanficProjects.find(p => p.id === projectId);
        if (project) {
            project.generatedContent = null;
            if (uiElements['generated-fic-output']) (uiElements['generated-fic-output']).value = 'Manuscript (if complete) or previous AI text cleared. Ready to re-finalize if manuscript is done.';
            if (uiElements['post-fic-button']) (uiElements['post-fic-button']).disabled = true;
            if (uiElements['submit-to-event-button']) (uiElements['submit-to-event-button']).disabled = true; // Re-disable submit too
            if (uiElements['finalize-fic-button'] && project.progress >= project.targetProgress) {
                 (uiElements['finalize-fic-button']).disabled = false;
            }
            showNotification("Generated fic output cleared. You can re-finalize if manuscript is complete.", 'info');
        }
    });
    
    (uiElements['fic-paid-checkbox'])?.addEventListener('change', (e) => {
    ...
    (uiElements['save-general-settings-button'])?.addEventListener('click', handleSaveGeneralProfileSettings);
    (uiElements['add-new-fandom-set-button'])?.addEventListener('click', () => handleEditFandomSetForm());
    (uiElements['save-fandom-set-button'])?.addEventListener('click', handleSaveFandomSet);
    (uiElements['cancel-edit-fandom-set-button'])?.addEventListener('click', () => {
        (uiElements['edit-fandom-set-form']).style.display = 'none';
    });
    (uiElements['edit-fandom-relationship-type'])?.addEventListener('change', (e) => {
        const form = (e.target).closest('#edit-fandom-set-form');
        if(!form) return;
        const relationship = (e.target).value;
        const workTitleInput = form.querySelector('#edit-fandom-work-title-input');
        const workDescInput = form.querySelector('#edit-fandom-work-desc-input');
        const favCharInput = form.querySelector('#edit-fandom-fav-char-input');
        const favCharDescInput = form.querySelector('#edit-fandom-fav-char-desc-input');
        const baseWorkGroup = form.querySelector('#select-base-fandom-set-group');
        const popularityTierGroup = form.querySelector('#edit-fandom-popularity-tier-group');
        const baseWorkSelect = form.querySelector('#select-base-fandom-set');
    ...
    (uiElements['select-base-fandom-set'])?.addEventListener('change', (e) => {
        const form = (e.target).closest('#edit-fandom-set-form');
        if(!form) return;
        const baseFandomId = (e.target).value;
        const relationship = (form.querySelector('#edit-fandom-relationship-type')).value;
        const workTitleInput = form.querySelector('#edit-fandom-work-title-input');
        const workDescInput = form.querySelector('#edit-fandom-work-desc-input');
        const favCharInput = form.querySelector('#edit-fandom-fav-char-input');
        const favCharDescInput = form.querySelector('#edit-fandom-fav-char-desc-input');
    ...
    (uiElements['open-player-custom-sns-post-modal-button'])?.addEventListener('click', () => {
        updatePlayerSnsPostCooldownMessage(); // Ensure cooldown message is current
        showArea('player-custom-sns-post-modal');
    });
    (uiElements['player-custom-sns-post-content'])?.addEventListener('input', (e) => {
        const content = (e.target).value;
        const remaining = 280 - content.length;
        const charCountEl = uiElements['player-custom-sns-post-char-count'];
        if(charCountEl) charCountEl.textContent = `${remaining} characters remaining`;
    });
    (uiElements['submit-player-custom-sns-post-button'])?.addEventListener('click', handleSubmitPlayerCustomSnsPost);
    (uiElements['view-message-log-button'])?.addEventListener('click', () => {
        renderMessageLog();
        showArea('message-log-area');
    });
    (uiElements['close-prologue-button'])?.addEventListener('click', () => {
        showArea(null); saveGameState(); // Save that prologue was shown
    });
    (uiElements['tutorial-friend-toggle-main'])?.addEventListener('change', (e) => {
        gameState.tutorialFriendEnabled = (e.target).checked;
        const senpai = gameState.npcs.find(n => n.isMotivationalFriend);
    ...
    (uiElements['confirm-doujinshi-printing-button'])?.addEventListener('click', handleConfirmDoujinshiPrinting);
    (uiElements['doujinshi-paper-quality'])?.addEventListener('change', updateDoujinshiCostEstimate);
    (uiElements['doujinshi-cover-quality'])?.addEventListener('change', updateDoujinshiCostEstimate);
    (uiElements['doujinshi-print-run-size'])?.addEventListener('input', updateDoujinshiCostEstimate);
